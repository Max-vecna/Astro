...
export function openEditMessageModal(msgId, originalText) {
    const modal = DOMElements.genericModal;
    // Sobrescreve HTML do modal genérico para edição
    modal.innerHTML = `
        <div class="glassmorphism p-6 rounded-2xl max-w-md w-full flex flex-col gap-4">
            <div class="flex justify-between items-center">
                <h3 class="text-lg font-bold">Editar mensagem</h3>
            ...
            <div class="flex gap-2 justify-end">
                <button id="cancel-edit" class="px-4 py-2 rounded-lg bg-slate-700">Cancelar</button>
                <button id="confirm-edit" class="px-4 py-2 rounded-lg bg-[var(--primary-color)] font-bold">Salvar</button>
            </div>
        </div>
    `;
    ...
    modal.querySelector('#cancel-edit').onclick = close;
    
    modal.querySelector('#confirm-edit').onclick = async () => {
        const newText = modal.querySelector('#edit-message-text').value;
...
export function openTranslateToModal(originalText) {
    ...
    modal.innerHTML = `
        <div class="glassmorphism p-6 rounded-2xl max-w-sm w-full flex flex-col gap-4">
            <div class="flex justify-between items-center">
                <h3 class="text-lg font-bold">Traduzir para</h3>
                <button class="close-modal text-xl">&times;</button>
            </div>
            ...
            <button id="confirm-translate-btn" class="bg-[var(--primary-color)] text-white p-3 rounded-xl font-bold">Traduzir</button>
        </div>
    `;
    modal.classList.remove('hidden'); modal.classList.add('flex');

    modal.querySelector('.close-modal').onclick = () => modal.classList.add('hidden');
    modal.querySelector('#confirm-translate-btn').onclick = async () => {
        const lang = modal.querySelector('#translate-target-lang').value;
        modal.classList.add('hidden');
        
        // Reabre modal com resultado
        showGenericModal('Tradução', '<i class="fas fa-circle-notch fa-spin"></i> Traduzindo...');
        try {
            const translated = await Services.traduzir(originalText, lang);
            DOMElements.genericModalContent.textContent = translated.texto;
        } catch (e) {
            DOMElements.genericModalContent.textContent = 'Erro ao traduzir.';
        }
    };
}
...
export async function handleAction(type, text, msgId = null) {
    const targetLang = state.currentTranslationLang; 
